  ...
  Papa.parse(file, {
    ...
    complete: results => {
      ...
      data = results.data
        ...
        .map(d => {
          // Normalize IDs and parent references
          const clean = str => str ? str.toString().trim() : '';
          const pad = id => id ? id.padStart(3, '0') : ''; // optional if you use zero-padded IDs

          return {
            ...d,
            id: pad(clean(d.id)),
            father: pad(clean(d.father)),
            mother: pad(clean(d.mother))
          };
        });
  ...
  const nodes = birdsToShow.map(b=> {
    ...
    return {
      ...
      font: {
        ...
        size: 16,
        // Enables displaying <b>text</b> in the label as bold text
        multi: 'html'
      }
    };
  });
